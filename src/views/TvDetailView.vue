<template>
  <div class="pt-20 md:pt-0 relative -z-10">
    ...
    <div class="z-10 pt-28 absolute top-0 w-full p-5 md:px-12 h-full bg-slate-900/90 grid grid-cols-[auto_1fr] gap-3 md:gap-8 lg:gap-12 content-center">
      ...
      <div class="text-left justify-self-stretch">
        <div class="font-bold md:text-lg lg:text-5xl text-slate-200">{{ movie.name }}</div>
  ...
  <div class="py-12">
    <div class="text-center">
      <div v-if="store.isWatchListExist(movie, 'tv') < 0" @click="store.addToWatchList(movie, 'tv')" class="inline-block p-2 py-2 text-sm text-slate-900 rounded-md bg-slate-200 self-center">Add to watchlist</div>
      <div v-if="store.isWatchListExist(movie, 'tv') > -1" @click="store.removeWatchList(movie, 'tv')" class="inline-block p-2 py-2 text-sm  rounded-md self-center text-white bg-red-600">Added</div>
    </div>
  </div>
