<template>
  <nav :class="isDetail()">
    <div class="flex gap-12">
      ...
      <div class="hidden md:flex gap-5">
        ...
        <router-link :to="{name: 'tv-shows'}">
          <span class="font-bold">TV Shows </span>
        </router-link>
      </div>
    </div>
    <div class="flex gap-3">
      ...
      <i @click="store.toggleMenuModal()" class='bx bx-dots-vertical-rounded md:hidden text-3xl'></i>
    </div>
  </nav>
</template>
...
export default {
  ...
  methods: {
    isDetail() {
      let classes =
        "w-full py-5 px-5 md:px-12 flex justify-between items-center z-0 ";

      if (this.$route.params.id) {
        classes += "fixed";
      } else {
        classes += "sticky top-0 backdrop-blur bg-slate-900/70";
      }
